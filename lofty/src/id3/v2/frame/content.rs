...
use crate::id3::v2::{
	BinaryFrame, ChapterFrame, ChapterTableOfContentsFrame, Frame, FrameFlags, FrameId,
};
...
pub(super) fn parse_content<R: Read>(
	...
	Ok(match id.as_str() {
		...
		"UFID" => UniqueFileIdentifierFrame::parse(reader, flags, parse_options.parsing_mode)?.map(Frame::UniqueFileIdentifier),
		"RVA2" => RelativeVolumeAdjustmentFrame::parse(reader, flags, parse_options.parsing_mode)?.map(Frame::RelativeVolumeAdjustment),
		...
		"TDEN" | "TDOR" | "TDRC" | "TDRL" | "TDTG" => TimestampFrame::parse(reader, id, flags, parse_options.parsing_mode)?.map(Frame::Timestamp),
		...
		"CHAP" => Some(Frame::Chapter(ChapterFrame::parse(reader, parse_options, version, flags)?)),
		"CTOC" => Some(Frame::TableOfContents(ChapterTableOfContentsFrame::parse(reader, parse_options, version, flags)?)),
