...
use aud_io::mp4::{AtomIdent, AtomInfo, AtomReader, AudioObjectType, SAMPLE_RATES};
	...
	if descriptor.tag == ELEMENTARY_DESCRIPTOR_TAG {
		...
		if descriptor.tag == DECODER_CONFIG_TAG {
			...
			if descriptor.tag == DECODER_SPECIFIC_DESCRIPTOR_TAG {
				...
				let Ok(extended_audio_object_type) = AudioObjectType::try_from(object_type) else {
					decode_err!(@BAIL Mp4, "Encountered an invalid audio object type");
				};

				properties.extended_audio_object_type = Some(extended_audio_object_type);
