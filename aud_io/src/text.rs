...
pub fn read_to_terminator<R>(reader: &mut R, encoding: TextEncoding) -> (Vec<u8>, usize)
where
	R: Read,
{
...
fn trim_end_nulls(text: &mut String) {
	if text.ends_with('\0') {
...
#[cfg(test)]
mod tests {
	use crate::text::{TextDecodeOptions, TextEncoding};
	use std::io::Cursor;
